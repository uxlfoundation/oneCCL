...
int main(int argc, char *argv[]) {
    ...
    std::vector<int> send_counts = { 1, 2, 16, 17, 1024, 1025 };
    ...
    for (auto activate_send_count : send_counts) {
        ...
        // active ones should send a vector of given size
        // others should always send vector of size one
        int send_count = is_activate(rank) ? activate_send_count : 1;
        ...
        for (size_t i = 0; i < recv_counts.size(); i++) {
            recv_counts[i] = is_activate(i) ? activate_send_count : 1;
        ...
        for (size_t rank_idx = 0; rank_idx < (size_t)size; rank_idx++) {
            int rank_plus_1 = rank_idx + 1;
            int rank_plus_1_sq = rank_plus_1 * rank_plus_1;
            if (is_activate(rank_idx)) {
                for (size_t i = 0; i < (size_t)activate_send_count; i++, total_received_count++) {
                    int value = rank_plus_1_sq + i;
                    if (recv_buf_host[offset++] != value) {
                        ++failure_count;
                        std::cout << "FAILED: expected: " << value
                                  << ", found: " << recv_buf_host[offset - 1] << std::endl;
                        break;
                    }
                }
            }
            else if (recv_counts[rank_idx] != 1) {
                ++failure_count;
                std::cout
                    << "FAILED: receive counts of a buffer that should contain one entry is not one\n";
                break;
            }
            else if (recv_buf_host[offset++] != rank_plus_1_sq) {
                ++failure_count;
                std::cout << " FAILED: expected: " << rank_plus_1_sq
                          << ", found: " << recv_buf_host[offset - 1] << std::endl;
                break;
            }
            else {
        ...
        size_t expected_count = (size_t)(activate_send_count * (size / 2) + (size - size / 2));
        if (total_received_count != expected_count) {
            ++failure_count;
            std::cout << "FAILED: incorrect total received count: " << total_received_count
                      << " vs " << expected_count << "\n";
            break;
        }
    }
