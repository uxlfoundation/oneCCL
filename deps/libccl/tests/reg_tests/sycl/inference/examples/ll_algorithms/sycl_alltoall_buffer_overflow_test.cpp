...
int alltoall_test(size_t element_count,
                  size_t offset_bytes,
                  queue &q,
    ...
    size_t total_element_count = element_count * comm_size;
    size_t main_data_bytes = total_element_count * sizeof(int32_t);
    ...
    uint8_t *send_buf =
        sycl::aligned_alloc_device<uint8_t>(4 * 1024, allocation_size, q); // +1 for safety
    ...
    std::cout << "Rank " << rank << ": element_count=" << element_count
              << ", offset_bytes=" << offset_bytes << ", element_count=" << element_count
              << ", comm_size=" << comm_size << ", allocation_size=" << allocation_size
              << std::endl;
    ...
    for (size_t i = 0; i < total_element_count; ++i) {
        ...
        write_by_byte<int32_t>(send_buf_cpu.data(), rank * total_element_count + i, t_offset_bytes);

        size_t target_rank = i / element_count;
        size_t local_index = i % element_count;

        size_t expected_recv = 0;
        expected_recv = target_rank * total_element_count + rank * element_count + local_index;
    ...
    std::cout << "Rank " << rank << ": alltoall start" << std::endl;

    ccl::alltoall((void *)(send_buf + offset_bytes),
    ...
    std::cout << "Rank " << rank << ": alltoall completed" << std::endl;
...
int main(int argc, char *argv[]) {
    ...
    for (size_t offset_bytes : offsets) {
        for (size_t element_count : element_counts) {
            ...
            int local_ret =
                alltoall_test(element_count, offset_bytes, q, comm, stream, comm_size, rank);
