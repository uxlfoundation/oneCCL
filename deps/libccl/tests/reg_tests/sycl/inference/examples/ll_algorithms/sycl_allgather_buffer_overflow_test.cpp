...
int allgather_test(sycl::queue &q,
    ...
    size_t recv_element_count = element_count * comm_size;
    size_t send_main_data_bytes = element_count * sizeof(int32_t);
    size_t recv_main_data_bytes = recv_element_count * sizeof(int32_t);
    ...
    std::cout << "Rank " << rank << ": element_count=" << element_count
              ...
              << ", send_allocation_size=" << send_allocation_size
              << ", recv_allocation_size=" << std::endl;
    ...
    for (size_t i = 0; i < element_count; ++i) {
        ...
        write_by_byte<int32_t>(send_buf_cpu.data(), rank * element_count + i, t_offset_bytes);
    }

    for (size_t i = 0; i < recv_element_count; ++i) {
        size_t t_offset_bytes = i * sizeof(int32_t) + offset_bytes;
        size_t expected_recv = i;
    ...
    std::cout << "Rank " << rank << ": allgather start" << std::endl;

    ccl::allgather((void *)(send_buf + offset_bytes),
    ...
    std::cout << "Rank " << rank << ": allgather completed" << std::endl;
...
int main(int argc, char *argv[]) {
    ...
    for (size_t offset_bytes : offsets) {
        for (size_t element_count : element_counts) {
            ...
            int local_ret = allgather_test(q,
                                           element_count,
                                           offset_bytes,
                                           ccl::datatype::int32,
                                           comm,
