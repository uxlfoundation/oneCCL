...
int pt2pt(size_t base_size,
    ...
    std::vector<uint8_t> expected_recv_cpu(allocation_size);
    ...
    for (size_t i = 0; i < base_size; ++i) {
        ...
        write_by_byte<uint32_t>(
            expected_recv_cpu.data(), 1 + size + ((rank + 1) % 2) + i, t_offset_bytes);
    }
    ...
    for (size_t t_offset_bytes = base_size_bytes + offset_bytes, k = 0;
        ...
        write_by_byte<uint32_t>(expected_recv_cpu.data(), recv_canary[k], t_offset_bytes);
    }
    ...
    if (rank == 0) {
        ccl::send(
            (void *)(send_buf + offset_bytes), base_size, ccl::datatype::uint32, 1, comm, stream)
            .wait();
        ccl::recv(
            (void *)(recv_buf + offset_bytes), base_size, ccl::datatype::uint32, 1, comm, stream)
            .wait();
    }
    if (rank == 1) {
        ccl::recv(
            (void *)(recv_buf + offset_bytes), base_size, ccl::datatype::uint32, 0, comm, stream);
        ccl::send(
            (void *)(send_buf + offset_bytes), base_size, ccl::datatype::uint32, 0, comm, stream)
            .wait();
    }
    ...
    /* print out the result of the test on the host side */
    {
        ...
        for (i = 0; i < allocation_size; ++i) {
            ...
            uint32_t expected = expected_recv_cpu[i];
...
int main(int argc, char *argv[]) {
    ...
    sycl::queue q(devices[rank % devices.size()], props); // case - more (unused) ranks than devices
    ...
    std::vector<size_t> offsets{ 0, sizeof(uint32_t), 4, 8 };
    for (size_t offset_bytes : offsets) {
        // smallest size that is fully and correctly divided without a remainder
        if (ret == 0)
            ret = pt2pt(16, offset_bytes, q, comm, stream, size, rank);
        // other cases
        if (ret == 0)
            ret = pt2pt(4, offset_bytes, q, comm, stream, size, rank);
        if (ret == 0)
            ret = pt2pt(1, offset_bytes, q, comm, stream, size, rank);
        if (ret == 0)
            ret = pt2pt(7, offset_bytes, q, comm, stream, size, rank);
        if (ret == 0)
            ret = pt2pt(8, offset_bytes, q, comm, stream, size, rank);
        if (ret == 0)
            ret = pt2pt(15, offset_bytes, q, comm, stream, size, rank);
        if (ret == 0)
            ret = pt2pt(16, offset_bytes, q, comm, stream, size, rank);
        if (ret == 0)
            ret = pt2pt(31, offset_bytes, q, comm, stream, size, rank);
        if (ret == 0)
            ret = pt2pt(32, offset_bytes, q, comm, stream, size, rank);
        if (ret == 0)
            ret = pt2pt(56, offset_bytes, q, comm, stream, size, rank);
        if (ret == 0)
            ret = pt2pt(57, offset_bytes, q, comm, stream, size, rank);
        if (ret == 0)
            ret = pt2pt(58, offset_bytes, q, comm, stream, size, rank);
        if (ret == 0)
            ret = pt2pt(59, offset_bytes, q, comm, stream, size, rank);
        if (ret == 0)
            ret = pt2pt(63, offset_bytes, q, comm, stream, size, rank);
        if (ret == 0)
            ret = pt2pt(64, offset_bytes, q, comm, stream, size, rank);
        if (ret == 0)
            ret = pt2pt(4096 - 7, offset_bytes, q, comm, stream, size, rank);
        if (ret == 0)
            ret = pt2pt(4096 - 6, offset_bytes, q, comm, stream, size, rank);
        if (ret == 0)
            ret = pt2pt(4096 - 5, offset_bytes, q, comm, stream, size, rank);
        if (ret == 0)
            ret = pt2pt(4096 - 4, offset_bytes, q, comm, stream, size, rank);
        if (ret == 0)
            ret = pt2pt(4096 - 2, offset_bytes, q, comm, stream, size, rank);
        if (ret == 0)
            ret = pt2pt(4096 - 1, offset_bytes, q, comm, stream, size, rank);
        if (ret == 0)
            ret = pt2pt(4096, offset_bytes, q, comm, stream, size, rank);
        if (ret == 0)
            ret = pt2pt(4096 + 1, offset_bytes, q, comm, stream, size, rank);
        if (ret == 0)
            ret = pt2pt(40960, offset_bytes, q, comm, stream, size, rank);
        if (ret == 0)
            ret = pt2pt(40961, offset_bytes, q, comm, stream, size, rank);
        if (ret == 0)
            ret = pt2pt(40962, offset_bytes, q, comm, stream, size, rank);
        if (ret == 0)
            ret = pt2pt(40963, offset_bytes, q, comm, stream, size, rank);
        if (ret == 0)
            ret = pt2pt(409600, offset_bytes, q, comm, stream, size, rank);
        if (ret == 0)
            ret = pt2pt(409601, offset_bytes, q, comm, stream, size, rank);
        if (ret == 0)
            ret = pt2pt(409602, offset_bytes, q, comm, stream, size, rank);
        if (ret == 0)
            ret = pt2pt(409603, offset_bytes, q, comm, stream, size, rank);
        if (ret == 0)
            ret = pt2pt(4096000, offset_bytes, q, comm, stream, size, rank);
        if (ret == 0)
            ret = pt2pt(4096001, offset_bytes, q, comm, stream, size, rank);
        if (ret == 0)
            ret = pt2pt(4096002, offset_bytes, q, comm, stream, size, rank);
        if (ret == 0)
            ret = pt2pt(4096003, offset_bytes, q, comm, stream, size, rank);
    }
