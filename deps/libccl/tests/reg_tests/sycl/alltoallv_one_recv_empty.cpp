...
int main(int argc, char *argv[]) {
    ...
    /* rank 0 receives nothing */
    size_t recv_size = rank == 0 ? 0 : (size * (size - 1) / 2);
    ...
    for (int i = 1; i < size; i++) {
        recv_counts[i] = (rank == 0) ? 0 : i + 1;
    }

    // set counts to zero - first rank should not receive anything
    send_counts[0] = 0;
