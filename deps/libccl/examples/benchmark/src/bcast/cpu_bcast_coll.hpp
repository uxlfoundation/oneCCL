...
struct cpu_broadcast_coll : cpu_base_coll<Dtype, broadcast_strategy_impl> {
    ...
    virtual void prepare_internal(size_t elem_count,
        ...
        for (size_t b_idx = 0; b_idx < base_coll::get_buf_count(); b_idx++) {
            for (size_t e_idx = 0; e_idx < elem_count; e_idx++) {
                if (comm.rank() == COLL_ROOT) {
                    ((Dtype*)send_bufs[b_idx][rank_idx])[e_idx] =
                        get_val<Dtype>(static_cast<float>(b_idx));
                }
                else {
                    ((Dtype*)send_bufs[b_idx][rank_idx])[e_idx] = 0;
                }
            }
        }
    }
    ...
    virtual void finalize_internal(size_t elem_count,
        ...
        for (size_t b_idx = 0; b_idx < base_coll::get_buf_count(); b_idx++) {
            for (size_t e_idx = 0; e_idx < elem_count; e_idx++) {
                Dtype expected = get_val<Dtype>(static_cast<float>(b_idx));
                if (comm.rank() == COLL_ROOT) {
                    Dtype value = ((Dtype*)send_bufs[b_idx][rank_idx])[e_idx];
                    if (base_coll::check_error<Dtype>(value, expected, comm)) {
                ...
                Dtype value = ((Dtype*)recv_bufs[b_idx][rank_idx])[e_idx];
                if (base_coll::check_error<Dtype>(value, expected, comm)) {
                    std::cout << this->name() << " recv_bufs: buf_idx " << b_idx << ", rank_idx "
                              << rank_idx << ", elem_idx " << e_idx << ", expected " << expected
                              << ", got " << value << std::endl;
