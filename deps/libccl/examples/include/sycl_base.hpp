...
// For open-source SYCL compilers, assume modern SYCL 2020 API
#if defined(SYCL_LANGUAGE_VERSION) && (SYCL_LANGUAGE_VERSION >= 202000L)
#define CCL_USE_SYCL121_API 0
#else
#define CCL_USE_SYCL121_API 1
#endif
...
struct test_args {
    ...
    static const size_t DEFAULT_LOOPS = 1;
    static const size_t MAX_LOOPS     = 10000;
    ...
    void print_help(const char* test) const {
        std::cout << "Usage: " << test << " [--queue_type <queue_type>] [--count <data_count>] [--loops <loop_count>] [--verbose]\n";
        std::cout << "Options:\n";
        std::cout
        ...
        std::cout << " --loops <loop_count>  Specify the number of loops (1-10000)\n";
    ...
    void parse() {
        ...
        if (args_map.find("loops") != args_map.end()) {
            std::string loops_str = args_map["loops"];
            loops = std::stoi(loops_str);
            if (loops < 1 || loops > MAX_LOOPS) {
                std::cout << "Invalid loops argument: " << loops << ". Must be between 1 and " << MAX_LOOPS << ". defaulting to " << DEFAULT_LOOPS << std::endl;
                loops = DEFAULT_LOOPS;
            }
        }

        if (args_map.find("verbose") != args_map.end()) {
            is_verbose = true;
        }
    };
};
