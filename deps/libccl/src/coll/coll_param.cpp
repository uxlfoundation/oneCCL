...
ccl_coll_attr::ccl_coll_attr(const ccl::scatter_attr& attr) {
    COPY_COMMON_OP_ATTRS(attr, this);
}
...
bool ccl_coll_param::is_inplace(buf_type type) const {
    ...
    if (ctype == ccl_coll_scatter) {
        return ccl::is_scatter_inplace(send_buf_ptr,
                                       recv_buf_ptr,
                                       get_recv_count(),
                                       dtype.size(),
                                       comm->rank(),
...
std::vector<void*> ccl_coll_param::get_all_non_zero_bufs() const {
    ...
    // Root check is reused by broadcast/scatter/reduce below; compute once.
    const bool is_root = (comm && root != CCL_INVALID_ROOT_RANK_IDX && comm->rank() == root);
    switch (ctype) {
        ...
        case ccl_coll_scatter: {
            /*
                For broadcast and scatter:
                - send_buf is only used on root rank
                  (scatter's send_buf holds comm_size * count elements)
...
void ccl_coll_param::validate() const {
    ...
    switch (ctype) {
        ...
        case ccl_coll_scatter:
            ...
            if (ctype == ccl_coll_reduce_scatter || ctype == ccl_coll_scatter) {
                /* reduce_scatter: every rank sends nranks blocks, receives 1
                 * scatter:        root sends nranks blocks, every rank receives 1
                 * in both cases:  send_count == recv_count * comm_size */
...
ccl_coll_param ccl_coll_param::create_scatter_param(const void* send_buf,
                                                    void* recv_buf,
                                                    size_t count,
                                                    ccl::datatype dtype,
                                                    int root,
    ...
    param.send_bufs.push_back(
        const_cast<void*>(send_buf)); // root: nranks*count elements; non-root: may be nullptr
    param.send_counts.push_back(
        comm->size() * count); // full send buffer = nranks * count (mirrors reduce_scatter)
    param.recv_bufs.push_back(recv_buf);
    param.recv_counts.push_back(count); // per-rank recv count
    param.root = root;
    param.set_common_fields(dtype, comm, stream, deps);
    param.validate();
