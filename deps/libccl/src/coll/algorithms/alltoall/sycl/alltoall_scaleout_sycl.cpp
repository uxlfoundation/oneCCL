...
sycl::event alltoall_scaleout_sycl_simple_blocking(sycl::queue& q,
    ...
    if (copy_to_host) {
        ...
        copy_e = q.submit([=](sycl::handler& h) {
            h.memcpy(
                scaleout_recv_buf, send_buf == ATL_IN_PLACE ? recv_buf : send_buf, size * world);
        });
        copy_e.wait();
    }
    ...
    ATL_CALL_THROW_IF_ERROR(
        atl_comm->alltoall(ep_idx, scaleout_send_buf, scaleout_recv_buf, size, req));
    ...
    if (copy_to_host) {
        op_end = q.memcpy(recv_buf, scaleout_recv_buf, size * world);
        op_end.wait();
    }
...
sycl::event alltoall_scaleout_sycl_simple_nonblocking(sycl::queue& q,
    ...
    if (copy_to_host) {
        scaleout_send_buf = ATL_IN_PLACE;
        scaleout_recv_buf = comm->get_scaleout_host_buf();
        copy_e = q.submit([=](sycl::handler& h) {
    ...
    if (copy_to_host) {
        op_end = q.submit([=](sycl::handler& h) {
            h.depends_on(op_end);
            h.memcpy(recv_buf, scaleout_recv_buf, size * world);
        });
    }
...
ccl::event alltoall_scaleout_sycl(sycl::queue& q,
    ...
    switch (algo) {
        case alltoall_scaleout_algo::direct: {
            ...
            if (copy_to_host &&
                count * ccl_dtype.size() * comm->size() > comm->get_scaleout_host_buf_size()) {
                LOG_DEBUG("alltoall count size = ",
                          count * ccl_dtype.size(),
                          " for ",
                          comm->size(),
                          " ranks exceeds scaleout_host_buf_size = ",
                          comm->get_scaleout_host_buf_size(),
                          " -- falling back");
                done = false;
                return e;
            }
