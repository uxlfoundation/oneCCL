...
ccl::status ccl_coll_build_topo_recv(ccl_sched* sched,
    ...
    if (ccl::global_data::env().ze_pt2pt_read) {
        ...
        LOG_DEBUG("build RECV: copy_entry is created");

        entry_factory::create<ack_report_entry>(
            sched, pt2pt_ack_tag, node_peer_rank, node_curr_rank, node_comm);
        LOG_DEBUG("build RECV: ack_report_entry is created");
    }
    else {
        LOG_DEBUG("build RECV: write mode is enabled");
        entry_factory::create<ack_accept_entry>(sched, pt2pt_ack_tag, node_peer_rank, node_comm);
        LOG_DEBUG("build RECV: ack_accept_entry is created");
    }
