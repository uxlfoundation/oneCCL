...
struct Send {
    ...
    Send(int nranks,
         ...
         bool p2p)
            : workSize(nelems * sizeof(T)),
              gr(calcAccessGranularity(input)),
              my_rank(my_rank),
              peer_rank(peer_rank),
              transmit(nranks,
                       input,
    ...
    static sycl::event launch(LLPatternData ll_pattern_data,
        ...
        if (LoadSeqNoInRuntime) {
            ...
            e1 = queue.submit([&](auto& h) { // port to sycl free functions
                h.parallel_for(
    ...
    void operator() [[sycl::reqd_sub_group_size(SubGroupSize)]] (sycl::nd_item<1> pos) const {
        ...
        const_cast<TransmitT*>(&transmit)->runSendStart(my_rank, peer_rank);
...
#endif
            const_cast<TransmitT*>(&transmit)->runSend(
...
private:
    static AccessGranularity calcAccessGranularity(T* input) {
        ...
        else if ((uintptr_t)input % sizeof(T) == 0) {
