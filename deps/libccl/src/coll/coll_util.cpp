...
bool is_scatter_inplace(const void* send_buf,
                        const void* recv_buf,
                        const size_t count,
                        const size_t dtype_size,
                        const size_t rank,
                        const size_t root,
                        const size_t comm_size) {
    // Matches MPI scatter semantics: in-place means recv_buf at the root
    // points to the root's own block inside send_buf, i.e.
    // recv_buf == send_buf + root * count * dsize. Non-root ranks do not
    // supply a send_buf, so they are never in-place.
    if (rank != root || send_buf == nullptr) {
        return false;
    }

    size_t block_size = count * dtype_size;
    const char* send_buf_start_ptr = reinterpret_cast<const char*>(send_buf);
    const char* send_buf_end_ptr = send_buf_start_ptr + block_size * comm_size;
    const char* recv_buf_ptr = reinterpret_cast<const char*>(recv_buf);

    bool address_aliasing =
        (recv_buf_ptr >= send_buf_start_ptr) && (recv_buf_ptr < send_buf_end_ptr);

    const char* own_block = send_buf_start_ptr + root * block_size;
    bool inplace = recv_buf_ptr == own_block;
    ...
    if (address_aliasing && !inplace) {
        ...
        std::string warning_msg("using scatter with a recv_buf that is not in-place"
                                " but that overlaps with send_buf! Did you mean to place your"
                                " recv_buf in the appropriate location within send_buf?");
        LOG_DEBUG("CAUTION: ", warning_msg);
        LOG_DEBUG("   send_buf ", ss_send.str());
