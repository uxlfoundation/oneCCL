...
ccl::status ccl_parallelizer::process_base(ccl_sched* sched, bool update_sched_id) {
    ...
    switch (coll_type) {
        ...
        case ccl_coll_scatter:
            /* scatter naive algorithm: no pipelining in this phase */
            part_count = 1;
            break;
        case ccl_coll_recv:
        case ccl_coll_send:
    ...
    switch (coll_type) {
        ...
        case ccl_coll_reduce_scatter:
            ...
            break;
        case ccl_coll_scatter:
            /* scatter with part_count==1: single chunk, no offset computation needed */
            counts[0] = coll_param.get_recv_count();
            offsets[0] = 0;
            break;
        case ccl_coll_alltoall:
        case ccl_coll_alltoallv:
    ...
    switch (coll_type) {
        ...
        case ccl_coll_scatter:
            /* part_count == 1 always; scatter does not chunk in this phase */
            for (idx = 0; idx < part_count; idx++) {
                ccl_coll_param param{ false };
