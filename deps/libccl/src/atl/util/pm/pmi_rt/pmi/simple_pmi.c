...
int PMI_Publish_name(const char service_name[], const char port[]) {
    ...
    if (PMI_initialized > SINGLETON_INIT_BUT_NO_PM) {
        MPL_snprintf(
            cmd, PMIU_MAXLINE, "cmd=publish_name service=%s port=%s\n", service_name, port);
...
int PMI_Spawn_multiple(int count,
    ...
    for (spawncnt = 0; spawncnt < count; spawncnt++) {
        ...
        rc = MPL_snprintf(
            tempbuf, PMIU_MAXLINE, "totspawns=%d\nspawnssofar=%d\n", count, spawncnt + 1);
        ...
        for (i = 0; i < preput_keyval_size; i++) {
            rc = MPL_snprintf(
                tempbuf, PMIU_MAXLINE, "preput_key_%d=%s\n", i, preput_keyval_vector[i].key);
            if (rc < 0) {
                return PMI_FAIL;
            }
            rc = MPL_strnapp(buf, tempbuf, PMIU_MAXLINE);
            ...
            rc = MPL_snprintf(
                tempbuf, PMIU_MAXLINE, "preput_val_%d=%s\n", i, preput_keyval_vector[i].val);
            if (rc < 0) {
                return PMI_FAIL;
            }
            rc = MPL_strnapp(buf, tempbuf, PMIU_MAXLINE);
...
static int PMII_getmaxes(int* kvsname_max, int* keylen_max, int* vallen_max) {
    char buf[PMIU_MAXLINE], cmd[PMIU_MAXLINE], errmsg[PMIU_ERROR_MSG_SIZE];
    int err, rc;
...
static int PMII_Connect_to_pm(char* hostname, int portnum) {
    ...
    if (connect(fd, (struct sockaddr*)&sa, sizeof(sa)) < 0) {
        switch (errno) {
            ...
            case EINPROGRESS: /*  (nonblocking) - select for writing. */ break;
