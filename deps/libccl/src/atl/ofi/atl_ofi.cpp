...
void atl_ofi::handle_address_table_update(atl_ofi_prov_t& prov,
    ...
    if (prov.sep) {
        prov.addr_table =
            (fi_addr_t*)realloc(prov.addr_table, new_size * sizeof(fi_addr_t) * ofi_ctx.ep_count);
    }
    else {
    ...
    if (prov.sep) {
        ...
        for (int i = 0; i < (int)new_ep_names_count; i++) {
            for (size_t j = 0; j < ofi_ctx.ep_count; j++) {
                prov.addr_table[(old_prov_ep_names_size + i) * ofi_ctx.ep_count + j] =
