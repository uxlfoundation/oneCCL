...
void topo_manager::build_host_info(int size, int rank, int global_current_id) {
    ...
    for (int rank_idx = 0; rank_idx < comm_size; rank_idx++) {
        const auto& rank_hostname = all_hostnames[rank_idx];
        int rank_host_idx =
            std::distance(unique_hostnames.begin(), unique_hostnames.find(rank_hostname));
        host_info_vec[rank_host_idx].ranks.insert(rank_idx);
    }
...
void topo_manager::post_init(int size, int rank, int global_current_id) {
    for (int rank_idx = 0; rank_idx < size; rank_idx++) {
        intra_card_colors[rank_idx] +=
            ccl::global_data::get()
                .shared_data->get_rank_info_vec_glob(global_current_id)[rank_idx]
                .host_idx *
            max_ranks_per_host;
        inter_card_colors[rank_idx] +=
            ccl::global_data::get()
                .shared_data->get_rank_info_vec_glob(global_current_id)[rank_idx]
                .host_idx *
            max_ranks_per_host;
