...
// TODO: can be queried
    ...
    void operator() [[sycl::reqd_sub_group_size(SubGroupSize)]] (sycl::nd_item<1> pos) const {
        // return;  // TODO temporary TOOD works here
        size_t nWires = pos.get_global_range(0) / SubGroupSize;
        size_t wireId_x = pos.get_global_id(0) / SubGroupSize / transmit.parallel_sg;

        size_t loopSize = nWires * ProtoT::wireCapacity / transmit.parallel_sg;
        size_t local_id = pos.get_sub_group().get_local_id()[0];

        for (size_t gOff = 0, tOff = 0; gOff < workSizeData.workSize; gOff += loopSize, ++tOff) {
            size_t wireOff = wireId_x * ProtoT::wireCapacity + gOff;

            ssize_t workLeft = workSizeData.workSize - wireOff;
            ...
            const_cast<TransmitT*>(&transmit)->runAllreduce(
                wireOff, tOff, workLeft, workSizeData, gr);
        }
    }
...
private:
    ...
    static WorkSizeDataT calcWorkSize(T* input, T* output, size_t size, int NRanks) {
        return WorkSizeDataT(NRanks, size, sizeof(T));
    }
