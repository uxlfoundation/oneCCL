...
struct ReduceScatterPhaseKernel {
    ...
    void operator()(sycl::nd_item<1> pos) const {
        reduce_scatter_wrap<T, vec_size>(send_buf, recv_buf, count, params);
    }
...
void allreduce_large_su_ring_write_single_kernel(const void *send_buf,
    ...
    ReduceScatterPhaseKernel<T, vec_size> rs_kernel(send_buf, recv_buf, count, rs_params);
    ...
    for (size_t nc = 0; nc < num_chunks; nc++) {
        ...
        for (int i = 0; i < N - 1; i++) {
            ...
            if (use_memcpy) {
                ...
                ReduceCopyKernel<T, vec_size> rc_kernel(out1, in1, in2, out2, data_count);
...
sycl::event allreduce_large_su_ring_write_no_ipc(const void *send_buf,
    ...
    if (count % N) {
        ...
        bool done;
        ccl::event e = allreduce_ll_ring((char *)send_buf + offset * dsize,
...
sycl::event allreduce_large_su_ring(const void *send_buf,
    ...
    if (read_algo) {
        ...
        for (int i = 0; i < N - 1; i++) {
            ...
            q.parallel_for(
                nd_range, [=](sycl::nd_item<1> it) [[sycl::reqd_sub_group_size(16)]] {
                    reduce_kernel_wrap<T, vec_size>(out, in1, in2, count_per_rank);
                });
#else
