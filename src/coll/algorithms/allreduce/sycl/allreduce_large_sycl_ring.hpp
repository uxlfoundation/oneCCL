...
void inline allgather_wrap(const void *send_buf,
    ...
    for (int i = 1; i < comm_size - 1; i++) {
        ...
        copy_kernel_wrap<T, vec_size>(dst, src, count_per_rank);
        // barrier
        p2p_barrier(params.flag_data, it, false, use_root_sync);
    }
}
...
void inline reduce_scatter_wrap(const void *send_buf,
    ...
    for (size_t nc = 0; nc < params.num_chunks; nc++) {
        ...
        for (int i = 0; i < comm_size - 1; i++) {
            ...
            reduce_copy_kernel_wrap<T, vec_size>(out1, in1, in2, out2, data_count, reduction);
...
void allreduce_large_su_ring_write_single_kernel(const void *send_buf,
    ...
    ReduceScatterPhaseKernel<T, vec_size> rs_kernel(
    ...
    AllgatherPhaseKernel<T, vec_size> ag_kernel(send_buf, recv_buf, count, params);
    ...
    for (size_t nc = 0; nc < num_chunks; nc++) {
        ...
        for (int i = 0; i < N - 1; i++) {
            ...
            if (use_memcpy) {
                ...
                ReduceCopyKernel<T, vec_size> rc_kernel(
    ...
    for (size_t nc = 0; nc < num_chunks; nc++) {
        ...
        for (int i = 0; i < N - 1; i++) {
            ...
            if (use_memcpy) {
                ...
                q.parallel_for(
                    nd_range_l, [=](sycl::nd_item<1> it) [[sycl::reqd_sub_group_size(16)]] {
                        reduce_copy_kernel_wrap<T, vec_size>(
                            out1, in1, in2, out2, data_count, reduction_op);
                    });
            }
        ...
        for (int i = 1; i < N - 1; i++) {
            ...
            if (use_memcpy) {
                ...
                q.parallel_for(
                    nd_range, [=](sycl::nd_item<1> it) [[sycl::reqd_sub_group_size(16)]] {
                        copy_2dst_wrap<T, vec_size>(local_dst, remote_dst, src, data_count);
                    });
            }
            invoke_p2p_barrier(node_comm, q, {}, is_cpu_barrier);
            slot = next_slot;
        ...
        // last step
        {
            ...
            if (use_memcpy) {
                ...
                q.parallel_for(
                    nd_range, [=](sycl::nd_item<1> it) [[sycl::reqd_sub_group_size(16)]] {
                        copy_kernel_wrap<T, vec_size>(local_dst, src, data_count);
                    });
            }
        }
    }
...
sycl::event allreduce_large_su_ring(const void *send_buf,
    ...
    static auto exe_bndl_cp =
        syclexp::get_kernel_bundle<copy_kernel_wrap<T, vec_size>, sycl::bundle_state::executable>(
            q.get_context());
    static sycl::kernel k_func_cp =
        exe_bndl_cp.template ext_oneapi_get_kernel<copy_kernel_wrap<T, vec_size>>();
    ...
    static sycl::kernel k_func_rs =
        exe_bndl_rs.template ext_oneapi_get_kernel<reduce_kernel_wrap<T, vec_size>>();

    static auto exe_bndl_rc =
        syclexp::get_kernel_bundle<reduce_copy_kernel_wrap<T, vec_size>,
    ...
    if (read_algo) {
        ...
        for (int i = 0; i < N - 1; i++) {
            ...
            q.parallel_for(
                nd_range, [=](sycl::nd_item<1> it) [[sycl::reqd_sub_group_size(16)]] {
                    reduce_kernel_wrap<T, vec_size>(out, in1, in2, count_per_rank, reduction_op);
                });
#else
