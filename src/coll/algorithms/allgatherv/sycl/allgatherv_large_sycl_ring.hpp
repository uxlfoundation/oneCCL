...
sycl::event allgatherv_large_su_ring(sycl::queue& q,
    ...
    LOG_DEBUG("allgatherv large ring kernel no tmp buffer");
    sycl::event e;
    int world = comm->size();
    int rank = comm->rank();
    std::shared_ptr<ccl_comm> node_comm = comm->get_node_comm();
    ...
    bool in_place =
        ccl::is_allgatherv_inplace(send_buf, send_count, recv_buf, recv_counts.data(), dsize, rank, world);
    ...
    for (int iter = 0; iter < nchunks; iter++) {
        ...
        if (use_single_kernel && !use_memcpy) {
