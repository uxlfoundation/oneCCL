...
SYCL_EXT_ONEAPI_FUNCTION_PROPERTY((syclexp::nd_range_kernel<1>))
#endif
void inline reduce_kernel_wrap(void *out, void *in1, void *in2, const size_t count, ccl_reduction_data reduction) {
    const sycl::nd_item<1> it = syclext::this_work_item::get_nd_item<1>();
    const size_t num_threads = it.get_global_range(0);
    const size_t req_threads = count / vec_size + count % vec_size;
...
#pragma unroll
    for (size_t offset = 0; offset < req_threads; offset += num_threads) {
        reduce<T, vec_size>(out, in1, in2, count, offset, reduction, it);
    }
}
...
#pragma unroll
    for (size_t offset = 0; offset < req_threads; offset += num_threads) {
        reduce_copy<T, vec_size>(out1, in1, in2, out2, count, offset, reduction, it);
    }
}
...
struct ReduceCopyKernel {
    ...
    void operator()(sycl::nd_item<1> pos) const {
        reduce_copy_kernel_wrap<T, vec_size>(out1, in1, in2, out2, count, reduction);
    }
            ...
            if (i < N - 2) {
                ...
                if (use_memcpy) {
                    ...
                    q.parallel_for(nd_range, [=](sycl::nd_item<1> it) {
                        reduce_kernel_wrap<T, vec_size>(out1, in1, in2, data_count, reduction_op);
                    });
#else
                    ...
                    q.parallel_for(
                        nd_range, [=](sycl::nd_item<1> it) [[sycl::reqd_sub_group_size(sg_size)]] {
                            reduce_copy_kernel_wrap<T, vec_size>(out1, in1, in2, out2, data_count, reduction_op);
                        });
#else
...
sycl::event reduce_scatter_large_su_ring_single_kernel(const void *send_buf,
    ...
    ReduceScatterKernel<T, vec_size> rs_kernel(send_buf, recv_buf, recv_count, reduction_op, params);
